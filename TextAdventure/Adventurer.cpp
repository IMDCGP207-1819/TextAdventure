...
std::shared_ptr<Item> Adventurer::DropItem(std::string itemName)
{
	// use the same style of find_if call to get the item from the inventory, if it exists.
	auto item = std::find_if(inventory.begin(), inventory.end(), [&itemName](std::shared_ptr<Item> &item) {return item->GetName() == itemName; });
	...
	// cache a reference to the actual item
