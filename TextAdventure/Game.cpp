...
void Game::Run() {
  ...
  while (true) {
    ...
    if (input.length() > 0) {
      ...
      // split the input string at every space ' ' found, storing the resulting
      // strings into a vector this is making use of the stringstream defaults.
    ...
    if (verb == "look") {
      ...
      // if there are two strings entered, we're probably trying to look at an
      // item
...
void Game::HandleItemTake(std::string item_name) {
  // we call DropItem on CurrentRoom, which either gives us back the item we
  // asked for or nullptr if it is not present.
...
void Game::HandleDropItem(std::string item_name) {
  // the call to DropItem on Adventurer (via the Player unique_ptr) either gives
  // us back a reference to the item, or nullptr.
  ...
  // otherwise, they aren't carrying the specified item, the same two reasons as
  // specified in HandleTakeItem apply here too
  std::cout << "You are not carrying that item\n";
}

void Game::PrintHelp() {
  // loop the commands in the command map, for each, print the "key" command and
  // the description.
...
void Game::LoadItemData() {
  ...
  for (auto it = itemList.begin(); it != itemList.end(); ++it) {
    json item = *it;
    items.push_back(std::make_unique<Item>(item["name"], item["description"],
                                           item["room_id"]));
  }

  for (auto &item : items) {
    if (item->GetTargetRoom() == 0)
      continue;

    int id = item->GetTargetRoom();

    auto room_find = std::find_if(
        rooms.begin(), rooms.end(),
        [id](const std::shared_ptr<Room> room) { return room->GetId() == id; });
...
void Game::LoadRoomData() {
  ...
  for (auto it = roomData.begin(); it != roomData.end(); ++it) {
    json room = *it;
    auto newRoom =
        std::make_shared<Room>(room["id"], room["name"], room["description"]);
  ...
  for (auto exit : exits) {
    ...
    if (startRoom && targetRoom) {
      startRoom->AddExit(
          std::make_shared<Exit>(GetDirectionFromString(exit.dir), targetRoom));
    }
  }
}
...
void Game::LoadConfig() {
  ...
  for (auto it = config.begin(); it != config.end(); ++it) {
    ...
    // each command has an array of synonyms, which need to be iterated over to
    // extract the values
    for (auto com = command["synonyms"].begin();
         com != command["synonyms"].end(); ++com) {
...
bool Game::SearchInventory(
  ...
  auto item_find =
      std::find_if(inventory.begin(), inventory.end(),
                   [item_name](const std::unique_ptr<Item> &item) {
  ...
  // the item exists in the players inventory, so we can print the description
  // and bail.
...
std::string Game::StandardiseCommandInput(std::string command) {
  ...
  for (it = commandMap.begin(); it != commandMap.end(); ++it) {
    ...
    for (auto word : words) {
      if (command == word) {
        return it->first; // if we do, we return the main command word for that
                          // synomym
      }
    }
  }
